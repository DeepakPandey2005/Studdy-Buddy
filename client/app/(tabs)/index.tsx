    ...
    <SafeAreaView className="flex-1 bg-gray-950" edges={['top']}>
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className="flex-row px-6 mb-8 justify-between">
          <View className="bg-gray-900 border border-gray-800 p-4 rounded-3xl flex-1 mr-3 relative overflow-hidden">
            <View className="absolute top-0 right-0 p-3 opacity-10">
              <Ionicons name="flame" size={60} color="#f59e0b" />
            </View>
            <Text className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">My Streak</Text>
            <View className="flex-row items-baseline">
              <Text className="text-white text-3xl font-bold mr-1">{stats.streak}</Text>
              <Text className="text-amber-500 font-bold">Days 🔥</Text>
            </View>
          </View>

          <View className="bg-gray-900 border border-gray-800 p-4 rounded-3xl flex-1 ml-3 relative overflow-hidden">
            <View className="absolute top-0 right-0 p-3 opacity-10">
              <Ionicons name="checkmark-circle" size={60} color="#10b981" />
            </View>
            <Text className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Steps Done</Text>
            <View className="flex-row items-baseline">
              <Text className="text-white text-3xl font-bold mr-1">{stats.completedSteps}</Text>
              <Text className="text-emerald-500 font-bold">/ {stats.totalSteps}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="mx-6 mb-8">
          <View className="bg-gradient-to-br from-indigo-900 via-gray-900 to-black border border-indigo-500/30 rounded-[40px] p-6 shadow-2xl shadow-indigo-500/20">
            <View className="flex-row justify-between items-center mb-6">
              <View>
                <Text className="text-white text-lg font-bold">Overall Progress</Text>
                <Text className="text-indigo-200 text-xs opacity-70">Keep pushing forward!</Text>
              </View>
              <View className="bg-indigo-500/20 px-3 py-1 rounded-full border border-indigo-500/30">
                <Text className="text-indigo-300 font-bold text-xs">{tasks.length} Active Goals</Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="relative items-center justify-center">
                <Svg width={140} height={140} className="rotate-[-90deg]">
                  ...
                  <Circle cx="70" cy="70" r="60" stroke="#1f2937" strokeWidth="12" fill="transparent" />
                  <Circle
                    cx="70"
                    cy="70"
                    r="60"
                ...
                <View className="absolute items-center justify-center">
                  <Text className="text-white text-3xl font-bold">{stats.progress}%</Text>
                </View>
              </View>

              <View className="flex-1 ml-6 space-y-4">
                <View>
                  <Text className="text-gray-400 text-xs uppercase mb-1">Tasks Completed</Text>
                  <Text className="text-white text-xl font-bold">{stats.tasksCompleted}</Text>
                </View>
                <View>
                  <Text className="text-gray-400 text-xs uppercase mb-1">Pending</Text>
                  <Text className="text-white text-xl font-bold">{stats.activeTasks}</Text>
                </View>
              </View>
            </View>
          </View>
